...
def run(n, report_prop, seed = None):
...
def gen_pref_dists(n):
    ...
    # TODO experiement with different SD scalings? 1.5 seems to work ok
    sdevs = np.random.random(n) * 1.5
...
def gen_prefs(n, k):
    ...
    rep_indices = set(np.random.choice(np.arange(n), size = k, replace = False))
